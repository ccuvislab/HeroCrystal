...
def freeze_layer_fedma(VGG, vgg_layer_idx):
    """Freeze layers for FedMA training"""
    conv_layer_list = [
        (1, 0),
        (1, 1),
        (2, 0),
...
def fedprox_aggregate_weights(model_list, global_weights=None, mu=0.01, keyword=None):
    """
    FedProx aggregation function compatible with FedUtils.avgWeight interface
    """
    parties = len(model_list)
    model_tmp = [None] * parties
    
    # Extract state dicts from models
    for idx, my_model in enumerate(model_list):
        model_tmp[idx] = my_model.state_dict()
    
    # Apply FedProx aggregation
    for key in model_tmp[0]:
        if (keyword is None) or (keyword in key):
            # Extract weights for this key
            client_weights = [model_tmp[i][key] for i in range(parties)]
            
            if global_weights is None:
                # Fallback to FedAvg if no global weights
                model_avg = torch.stack(client_weights, dim=0).mean(dim=0)
            else:
                # FedProx aggregation: (1-mu) * avg + mu * global
                model_avg = torch.stack(client_weights, dim=0).mean(dim=0)
                if key in global_weights:
                    model_avg = (1 - mu) * model_avg + mu * global_weights[key]
            
            # Update all models with aggregated weights
            for i in range(parties):
                model_tmp[i][key] = model_avg
    
    # Load aggregated weights back to models
...
def run_fedma_training(cfg, args, Trainer, source_dataset_list, model_path_list, teacher_trainer, output_folder):
    """Run FedMA training algorithm"""
    print("Running FedMA algorithm")
...
def run_fedavg_training(cfg, args, Trainer, source_dataset_list, initial_model_path, output_folder):
    """Run FedAvg/FedProx training algorithm"""
    algorithm = cfg.FEDSET.ALGORITHM.lower()
    ...
    mu = cfg.FEDSET.MU if hasattr(cfg.FEDSET, 'MU') else 0.01
    global_weights = None
    
    # Load initial global weights for FedProx
    if algorithm == "fedprox":
        if os.path.exists(initial_model_path):
            global_weights = torch.load(initial_model_path, map_location='cpu')
            print(f"Loaded global weights from {initial_model_path}")
        else:
            print("Warning: Initial model path not found, falling back to FedAvg")
    ...
    for r in range(ROUND):
        ...
        # Choose aggregation method based on algorithm
        if algorithm == "fedprox":
            avg_model = fedprox_aggregate_weights(model_list, global_weights, mu, keyword)
        else:
            avg_model = FedUtils.avgWeight(model_list, wk_ratio, keyword)

        # save model_list
        if algorithm == "fedprox":
            initial_model_path = os.path.join(output_folder, "FedProx_" + str(r) + ".pth")
        else:
            initial_model_path = os.path.join(output_folder, "FedAvg_" + str(r) + ".pth")
        torch.save(avg_model[0].state_dict(), initial_model_path)

        # Update global weights for next round (FedProx)
        if algorithm == "fedprox":
            global_weights = avg_model[0].state_dict()
            print(f"Updated global weights for round {r+1}")
...
def main(args):
    ...
    algorithm = cfg.FEDSET.ALGORITHM  # Should be "fedma" or "fedavg"
    ...
    if algorithm.lower() == "fedma":
        # FedMA specific setup
        model_path_list = cfg.MODEL.TEACHER_PATH
        teacher_trainer = cfg.MODEL.TEACHER_TRAINER
        
        model_list = run_fedma_training(
            cfg, args, Trainer, source_dataset_list, 
            model_path_list, teacher_trainer, output_folder
        )
        
        # Final averaging for FedMA
        wk_ratio = [1] * parties
        wk_ratio = [x / parties for x in wk_ratio]
        ...
        # save final model
        initial_model_path = os.path.join(output_folder, "FedMA_final.pth")
        torch.save(avg_model[0].state_dict(), initial_model_path)
        print("save avg model to {}".format(initial_model_path))

    elif algorithm.lower() in ["fedavg", "fedprox"]:
        # FedAvg/FedProx specific setup
        initial_model_path = cfg.MODEL.WEIGHTS
        
        model_list = run_fedavg_training(
            cfg, args, Trainer, source_dataset_list, 
            initial_model_path, output_folder
        )

    else:
        raise ValueError(f"Unknown algorithm: {algorithm}. Supported algorithms: 'fedma', 'fedavg', 'fedprox'")

    # close wandb for healthy
