...
def compute_cost(global_weights,   ### weights of largest shape.
                 weights_j,        ### weights of client j.
                 global_sigmas, 
                 sigma_inv_j, 
                 prior_mean_norm, 
                 prior_inv_sigma,
                 popularity_counts, ### list as long as the largest shape.
                 gamma, 
                 J):
    """ ### Hard to understand.
    """
    ...
    Lj = weights_j.shape[0]   ### weight shape of client j.
    counts = np.minimum(np.array(popularity_counts, dtype=np.float32), 10)  ### at most 10? Why???

    sij_p_gs = sigma_inv_j + global_sigmas
    red_term = (global_weights ** 2 / global_sigmas).sum(axis=1)
    stupid_line_start = time.time()

    param_cost = np.array([row_param_cost_simplified(global_weights, weights_j[l], sij_p_gs, red_term) 
                           for l in range(Lj)], dtype=np.float32)
    #logger.info("global weights shape: {}, weight j shape: {}, weight j(whole) shape: {}, global_sigmas: {}, sigma_inv_j: {}, param_cost shape: {}".format(global_weights.shape, weights_j[0].shape, weights_j.shape, global_sigmas.shape, sigma_inv_j.shape, param_cost.shape))
...
def matching_upd_j(weights_j,       ### weights of client j.
                   global_weights,  ### largest weights.
                   sigma_inv_j,     
                   global_sigmas, 
                   prior_mean_norm, 
                   prior_inv_sigma,
                   popularity_counts,   ### list as long as global_weights. 
                   gamma, 
                   J):

    L = global_weights.shape[0] ### number of nerons.

    compute_cost_start = time.time()
    
    full_cost = compute_cost(global_weights.astype(np.float32), 
    ...
    row_ind, col_ind = solve_dense(-full_cost)  ### find the minimal lost combination. row_ind = dim of matrix.
    ...
    new_L = L  # shape of global weights, >= original model.
...
def block_patching(w_j, 
    ...
    """
    """
        w_j    : weights of specific layer of client j.
        L_next : input dim for next layer.
    """
    ...
    ### layer_meta_data = shape of the layer = (input_dim, output_dim = [# of filters])
    ...
    if layer_type == "conv":    
        ...
        if network_name == "simple-cnn":
            ...
            shape_estimator = VGG11ContainerConvBlocks(num_filters=matching_shapes)
            
        if dataset in ("cifar10", "cinic10"):
#             dummy_input = torch.rand(1, 3, 32, 32)  ### cyjui
        ...
        block_indices = [np.arange(i*estimated_output.size()[-1]**2, 
                                   (i+1)*estimated_output.size()[-1]**2) for i in range(L_next)]
        
        #for i, bid in enumerate(block_indices):
        #    logger.info("{}, {}".format(i, bid))
        #logger.info("**"*20)
        
        ori_block_indices = [np.arange(i*estimated_output.size()[-1]**2, 
                                       (i+1)*estimated_output.size()[-1]**2) for i in range(prev_layer_meta_data[0])]
...
#                          dataset="cifar10",
    ...
    if layer_type == "conv":    
        ...
        if network_name == "vgg16":
            shape_estimator = VGG16ContainerConvBlocks(num_filters=matching_shapes)
...
def match_layer(weights_bias, sigma_inv_layer, mean_prior, sigma_inv_prior, gamma, it):
    """
    weigjts_bias    : weights & bias packed in one list(array).
    sigma_inv_layer :
    mean_prior      :
    """
    
    J = len(weights_bias)  ### number of clients.

    ### sorted weights(by client) from more channel to less.
    ...
    global_weights = prior_mean_norm + batch_weights_norm[group_order[0]] ### largest model.
    global_sigmas  = np.outer(np.ones(global_weights.shape[0]), 
                              sigma_inv_prior + sigma_inv_layer[group_order[0]])

    popularity_counts = [1] * global_weights.shape[0]  ### a list as long as the largest shape.

    assignment = [[] for _ in range(J)]  ### placeholder for clients.

    assignment[group_order[0]] = list(range(global_weights.shape[0]))  ### largest shape.

    ## Initialize
    for j in group_order[1:]:
        
        global_weights, global_sigmas, popularity_counts, assignment_j = matching_upd_j(batch_weights_norm[j], ### w shape of client j.
                                                                                        global_weights,        ### largest shape.
                                                                                        sigma_inv_layer[j],
                                                                                        global_sigmas, 
                                                                                        prior_mean_norm,
                                                                                        sigma_inv_prior,
                                                                                        popularity_counts, ### list as long as global_wts. 
                                                                                        gamma, 
                                                                                        J)
        assignment[j] = assignment_j  ### [order of filter] for each client.
        

    ## Iterate over groups
    for iteration in range(it): ### set to 5 in BBP_MAP.
        ...
        for j in random_order:  # random_order:
            ...
            to_delete = []
            ## Remove j
            Lj = len(assignment[j])  ### number of filter of client j.
            
            for l, i in sorted(zip(range(Lj), assignment[j]), key=lambda x: -x[1]): ### sorted by assignment, from big to small.
...
def layer_wise_group_descent(batch_weights,
                             layer_index,  ### always >= 1. 
                             batch_frequencies, 
                             sigma_layers, 
                             sigma0_layers, 
                             gamma_layers, 
    ...
    We implement a layer-wise matching here:
        sigma_layers, sigma0_layers, gamma_layers: are initialized as constants. 
    """
    
    ### scores for each layer??, cyjui. [21/11/05]
    ### create sigma, gama list as deep as the layers.
...
#     init_num_kernel = batch_weights[0][0].shape[0]  ### [net][layer][num_of_kernel][*kernel_size]
#     print("INIT_NUM_KERNEL: {:}, {:} ".format(batch_weights[0][0].shape, batch_weights[0][layer_index].shape))
    init_num_kernel = batch_weights[0][layer_index].shape[0]  ### [net][layer][num_of_kernel][*kernel_size]

    # for saving (#channel * k * k)  ### size of the weights(for a layer)
#     init_channel_kernel_dims = []
#     for bw in batch_weights[0]: ### bw => weights by layers.
#         if len(bw.shape) > 1: init_channel_kernel_dims.append(bw.shape[1])
    init_channel_kernel_dims = [bw.shape[1] for bw in batch_weights[0] if len(bw.shape) > 1]
    ...
    ### paramaters for the specific layer.
    ...
    if layer_index <= 1:  ### first layer.

        weights_bias = [np.hstack(
                                  (batch_weights[j][0], batch_weights[j][layer_index * 2 - 1].reshape(-1, 1))
                                  ) 
                        for j in range(J)]
    ...
    elif layer_index == (n_layers - 1) and n_layers > 2:  ### last layer ???
        ...
        # we switch to ignore the last layer here:  ### what's the difference????
        if first_fc_identifier:
            weights_bias = [np.hstack((batch_weights[j][2 * layer_index - 2].T, 
    ...
    elif (layer_index > 1 and layer_index < (n_layers - 1)): ### all layers between first and last.
        ...
        if 'conv' in layer_type or 'features' in layer_type:
            ...
            first_fc_identifier = (('fc' in layer_type or 'classifier' in layer_type) and 
                                   ('conv' in prev_layer_type or 'features' in prev_layer_type))
            #logger.info("first_fc_identifier: {}".format(first_fc_identifier))
            
            ### What's the difference????????????
            if first_fc_identifier:
#                 print("****************** First FC layer {:}, {:}".format(batch_weights[0][2 * layer_index - 2].shape,
                weights_bias = [np.hstack((batch_weights[j][2 * layer_index - 2].T, 
                                           batch_weights[j][2 * layer_index - 1].reshape(-1, 1))) for j in range(J)]
            else:
#                 print("****************** Not First FC layer ")
                weights_bias = [np.hstack((batch_weights[j][2 * layer_index - 2].T, 
                                           batch_weights[j][2 * layer_index - 1].reshape(-1, 1))) for j in range(J)]          
...
#################### End of calculating the MAP.############################
    
    assignment_c, global_weights_c, global_sigmas_c = match_layer(weights_bias, 
                                                                  sigma_inv_layer, mean_prior,
                                                                  sigma_inv_prior, gamma, 
                                                                  it)
    ### number of filter (as input dim) for next layer.
    L_next = global_weights_c.shape[0]
    
    print("!!!!!!! L_next : {:}, layer_index: {:}".format(L_next, layer_index))
    ...
    if layer_index <= 1:
        ...
        logger.info("Branch A, Layer index: {}, Global weights out shapes: {}".
                    format(layer_index, [gwo.shape for gwo in global_weights_out]))
        ...
        logger.info("#### Branch B, Layer index: {}, Global weights out shapes: {}".
                    format(layer_index, [gwo.shape for gwo in global_weights_out]))

    elif (layer_index > 1 and layer_index < (n_layers - 1)):
        
        layer_type = model_layer_type[2 * layer_index - 2]
        gwc_shape = global_weights_c.shape   ### gwc = global weight channel??? the upperbound of global weights.
        ...
        logger.info("Branch layer index, Layer index: {}, Global weights out shapes: {}".
                    format(layer_index, [gwo.shape for gwo in global_weights_out]))
    ...
    """
    map_out      : [global weights]/[global_inv_sigma] 
                   (because global_weights_out was normalized within [macth_layer])
    assignment_c : the matched permutation info for each client j.
    L_next       : channel of this layer, (input dim) for next layer to use.
    """
    return map_out, assignment_c, L_next


def layer_wise_group_descent_full_cnn(batch_weights,
                                      layer_index,  ### always >= 1. 
#                                      batch_frequencies, 
                                     sigma_layers, 
                                     sigma0_layers, 
...
#                                      matching_shapes,
    ...
    ### create sigma, gama list as deep as the layers.   
    if type(sigma_layers) is not list:
        sigma_layers = (n_layers - 1+1)  * [sigma_layers]
    if type(sigma0_layers) is not list:
        sigma0_layers = (n_layers - 1+1) * [sigma0_layers]
    if type(gamma_layers) is not list:
        gamma_layers = (n_layers - 1+1)  * [gamma_layers]
...
#     for bw in batch_weights[0]: ### bw => weights by layers.
#         if len(bw.shape) > 1: init_channel_kernel_dims.append(bw.shape[1])
    init_channel_kernel_dims = [bw.shape[1] for bw in batch_weights[0] if len(bw.shape) > 1]  ### the size of bias.
    ...
    if layer_index <= 1:  ### first layer.
        ...
        weights_bias = [np.hstack(
                                  (batch_weights[j][0],   ### weights.
                                   batch_weights[j][layer_index * 2 - 1].reshape(-1, 1))   ### bias.
                                  ) 
                        for j in range(J)]
...
#     elif (layer_index > 1 and layer_index < (n_layers - 1)): ### all layers between first and last.
    else:  ### all layers other than the first.
    
        layer_type = model_layer_type[2 * layer_index - 2]
        prev_layer_type = model_layer_type[2 * layer_index - 2 - 2]

#         if 'conv' in layer_type or 'features' in layer_type:
        if 'weight' in layer_type or 'features' in layer_type:
            weights_bias = [np.hstack((batch_weights[j][2 * layer_index - 2], 
                                       batch_weights[j][2 * layer_index - 1].reshape(-1, 1))) 
                            for j in range(J)]        
...
#     elif (layer_index > 1 and layer_index < (n_layers - 1)):
#     elif layer_index == (n_layers-1):   ### dealing with the last layer, 22/01/11.
        
#         layer_type = model_layer_type[2 * layer_index - 2] ### should be conv.
#         gwc_shape = (512, 512) 
#         L_next = 512
        
#         global_weights_out = [global_weights_c[:, :512], 
#                               global_weights_c[:, 512]]
#         global_inv_sigmas_out = [global_sigmas_c[:, :512], 
#                                  global_sigmas_c[:, 512]]
...
def layer_wise_group_descent_pfnm(batch_weights, layer_index, 
                                  batch_frequencies, sigma_layers, 
                                sigma0_layers, gamma_layers, it, 
                                model_meta_data, 
                                model_layer_type,
                                n_layers,
    ...
    if layer_index <= 1:
        ...
        layer_type = model_layer_type[2 * layer_index - 2]
        prev_layer_type = model_layer_type[2 * layer_index - 2 - 2]
        first_fc_identifier = (('fc' in layer_type or 'classifier' in layer_type) 
                               and ('conv' in prev_layer_type or 'features' in layer_type))
